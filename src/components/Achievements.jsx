...
import { internshipsData, openSourceData, achievementsData, codingProfilesData, certificationsData } from '../content.js';
        ...
        <div className="container-custom">
            ...
            <div className="flex flex-col space-y-8">
              {internshipsData.map((exp, index) => (
                <motion.div 
                  ...
                  className="list-row items-start flex-col md:flex-row px-4 -mx-4 rounded-xl transition-colors hover:bg-white/5"
                >
          ...
          <div className="mb-20">
            <motion.div 
              ...
              className="mb-12"
            >
              <h2 className="heading-section border-b border-cream/20 pb-8">OPEN SOURCE EXPERIENCE</h2>
            </motion.div>
            <div className="flex flex-col space-y-8">
              {openSourceData.map((exp, index) => (
                <motion.div 
                  key={index} 
                  initial={{ opacity: 0, y: 30 }} 
                  whileInView={{ opacity: 1, y: 0 }} 
            ...
            <div className="flex flex-col space-y-6">
              {achievementsData.map((ach, index) => (
                <motion.div 
                  ...
                  transition={{ duration: 0.7, delay: index * 0.1, ease: [0.16, 1, 0.3, 1] }} 
                  whileHover={{ scale: 1.01, x: 5 }}
                  className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-cream/10 pb-4 px-4 -mx-4 transition-colors hover:border-cream/30"
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {codingProfilesData.map((profile, index) => (
                <motion.a 
                  ...
                  whileTap={{ scale: 0.97 }}
                  className="block bg-white/5 p-6 rounded-xl hover:bg-white/10 transition cursor-pointer"
                >
